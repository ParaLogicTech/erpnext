...
from frappe.utils import getdate, get_first_day, add_years, get_year_start, flt
...
class SummarizedProfitAndLossReport:
	...
	def get_data(self):
		...
		data, raw_data = self.get_account_group_data(current_account_group)
		# for row in data:
		# 	if row.get('row_type') == 'Account Group':
		# 		row['account_group'] = row.get('account_group') or row.get('name')

		if not is_root:
			totals = {k: 0 for k in self.total_fields}
			for row in raw_data:
			...
			group_root_type = frappe.db.get_value("Account Group", current_account_group, "root_type")
			for key in totals:
	...
	def get_account_group_data(self, group_name):
		...
		for group_row in group.rows:
			if group_row.row_type == "Account Group":
				...
				child_totals = self.calculate_group_totals(child_group_doc)
		...
		running_totals = {key: 0 for key in self.total_fields}
		for group_row in group.rows:
			if group_row.row_type == "Account":
				account_data = self.get_account_row(group_row.account)
				data.append(account_data)

				for key in running_totals:
					running_totals[key] += flt(account_data.get(key))
	...
	def calculate_section_totals(self, row, child_groups, running_totals):
		...
		if included_categories == {"Income"} or included_categories == {"Expense"}:
			multiplier = -1 if included_categories == {"Expense"} else 1
			for group_info in included_groups:
				for key in section_totals:
					section_totals[key] += multiplier * group_info["totals"][key]
		else:
	...
	def calculate_group_totals(self, group):
		"""Calculate totals for all accounts in a group, recursively."""
		totals = {key: 0 for key in self.total_fields}
		...
		for account in accounts:
			account_data = self.get_account_row(account.name)

			for key in totals:
				totals[key] += flt(account_data.get(key))

		return totals

	def get_account_row(self, account):
		account_doc = frappe.get_doc("Account", account)

		balances = {
			"mtd_actual": self.get_account_balance(account, self.filters.month_start_date, self.filters.report_date),
			"ytd_actual": self.get_account_balance(account, self.filters.year_start_date, self.filters.report_date),
			"mtd_prev_year": self.get_account_balance(account, self.filters.prev_year_month_start, self.filters.prev_year_date),
			"ytd_prev_year": self.get_account_balance(account, self.filters.prev_year_start, self.filters.prev_year_date),

			"mtd_budget": self.get_budget_amount(account, self.filters.month_start_date, self.filters.report_date, self.filters.report_date.year),
			"ytd_budget": self.get_budget_amount(account, self.filters.year_start_date, self.filters.report_date, self.filters.report_date.year),
		}
	...
	def get_account_balance(self, account, start_date, end_date):
		...
		params = [account, self.filters.company, start_date, end_date]
