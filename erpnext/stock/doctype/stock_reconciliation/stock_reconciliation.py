...
class StockReconciliation(StockController):
	...
	def submit(self):
		if len(self.items) > 100:
			...
			self.queue_action('_submit', timeout=600)
		else:
			self._submit()

	def cancel(self):
		if len(self.items) > 100:
			self.queue_action('_cancel', timeout=600)
		else:
			self._cancel()
...
def get_items(args):
	...
	if qty_condition == "negative":
		...
		bin_qty_condition = "and ((bin.actual_qty > 0 and bin.actual_qty < 1) {0})".format(has_batch_no_condition)
